                ...
                foreach ($_SESSION["sepet"] as $urunID => $bilgi) {
                    ...
                    if($urunbilgisi!=false)
                    {
                            ...
                            if(!empty($_SESSION["sepetVaryasyon"]))
                            {
                                foreach ($_SESSION["sepetVaryasyon"][$urunbilgisi[0]["ID"]] as $secenekID => $secenekAdet) {
                                    ...
                                    if($stokTablo!=false)
                                    {
                                        ...
                                        $varyasyonStokDusme=$VT->SorguCalistir("UPDATE urunvaryasyonstoklari","SET stok=? WHERE ID=?",array("stok-".$secenekAdet["adet"],$stokTablo[0]["ID"]),1);
                                        $urunStokDusme=$VT->SorguCalistir("UPDATE urunler","SET stok=? WHERE ID=?",array("stok-".$secenekAdet["adet"],$urunbilgisi[0]["ID"]),1);
                        ...
                        else
                        {
                            ...
                            $urunStokDusme=$VT->SorguCalistir("UPDATE urunler","SET stok=? WHERE ID=?",array("stok-".$bilgi["adet"],$urunbilgisi[0]["ID"]),1);
