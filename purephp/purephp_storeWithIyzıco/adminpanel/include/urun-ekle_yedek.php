                        ...
                        if (!empty($_SESSION["varyasyonlar"]) && !empty($_SESSION["secenekler"]))
                        {
                            ...
                            switch ($varyasyonAdet){
                                case 1;
                                    ...
                                    {
                                        $varyasyonIDOgrenme=$VT->VeriGetir("urunvaryasyonlari","WHERE baslik=? AND urunID=?",array($varyasyon,$urunID));
                                        $secenekIDOgrenme=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE baslik=? AND urunID=?",array($_SESSION["secenekler"][$varyasyon][$w],$urunID));
                                        if($varyasyonIDOgrenme!=false && $secenekIDOgrenme!=false)
                                        {
                                            $varyasyonStok=$VT->filter($_POST["stok"][$w]);
                                            $stokEkle=$VT->SorguCalistir("INSERT INTO urunvaryasyonstoklari","SET urunID=?, varyasyonID=?, secenekID=?, stok=?, tarih=?",array($urunID,$varyasyonIDOgrenme[0]["ID"],$secenekIDOgrenme[0]["ID"],$varyasyonStok,date("Y-m-d")));
                                        }
                                    }
                                    break;
                                ...
                                case 2;
                                    ...
                                    for($w=0;$w<count($_SESSION["secenekler"][$varyasyon]);$w++)
                                    {
                                        $varyasyonIDOgrenme=$VT->VeriGetir("urunvaryasyonlari","WHERE baslik=? AND urunID=?",array($varyasyon,$urunID));
                                        $secenekIDOgrenme=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE baslik=? AND urunID=?",array($_SESSION["secenekler"][$varyasyon][$w],$urunID));
                                        for($r=0;$w<count($_SESSION["secenekler"][$varyasyon2]);$r++)
                                        {
                                            $varyasyonIDOgrenme2=$VT->VeriGetir("urunvaryasyonlari","WHERE baslik=? AND urunID=?",array($varyasyon2,$urunID));
                                            $secenekIDOgrenme2=$VT->VeriGetir("urunvaryasyonsecenekleri","WHERE baslik=? AND urunID=?",array($_SESSION["secenekler"][$varyasyon2][$r],$urunID));
                                            if($varyasyonIDOgrenme!=false && $secenekIDOgrenme!=false && $varyasyonIDOgrenme2!=false && $secenekIDOgrenme2!=false)
                                            {
                                                $varyasyonStok=$VT->filter($_POST["stok"][$r]);
                                                $stokEkle=$VT->SorguCalistir("INSERT INTO urunvaryasyonstoklari","SET urunID=?, varyasyonID=?, secenekID=?, stok=?, tarih=?",array($urunID,$varyasyonIDOgrenme[0]["ID"]."@".$varyasyonIDOgrenme2[0]["ID"],$secenekIDOgrenme[0]["ID"]."@".$secenekIDOgrenme2[0]["ID"],$varyasyonStok,date("Y-m-d")));
                                            }
                                        }
            ...
            <form action="#" method="post" class="urunEklemeFormu" enctype="multipart/form-data">
                <div class="row">
                        ...
                        <div class="card-body card card-primary">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <label>Stok</label>
                                        <input value="1" type="number" class="form-control" placeholder="Stok ..." name="genelstok" style="width:100px;" min="1">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                    ...
                    <div class="col-md-12" style="margin-bottom: 20px;">
                        <a class="btn btn-primary varyasyonEkleme" style="color: white;float: right;"><i class="fa fa-plus"></i> Varyasyon Ekle</a>
                        <style>
                            ...
                            .varyasyonGrup ul{display: block;list-style: none;padding: 3px;min-height: 100px; background: white; border: 1px solid #ddd;}
                        ...
                        <div class="row stokYonetimAlani" style="display: block;clear: both;">
                            ...
                            <div class="row butonuKontrolEt" style="display: none;clear: both;">
                                <a class="btn btn-warning" onclick="stokOlustur();" style="color: white">Varyasyon Stoklarını Oluştur</a>
                            </div>
                        </div>
